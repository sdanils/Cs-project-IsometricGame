...
{
    internal class TowerThree : AbstractTower
    {
        static public int cost = 1500;
        System.Windows.Forms.Timer timerShout;


        //Sizes sizes, int type, Point panelCoordCell, int Id
        public TowerThree(Sizes sizes, int type, Point panelCoordCell, int id, Point mapC, List<AbstractBullet> bullets, List<AbstractMonster> monsters, ListDrawnObjects listDrawn) : base(type, id, mapC, sizes, bullets, monsters, listDrawn)
        {
            string fName = null;
        ...
        public void Shout()
        {
            ...
            foreach (AbstractMonster monster in monsters)
            {
                ...
                if (distance < minDistance && monster.frozesFlag != true)
                {
            ...
            if (nearestMonster != null)
            {
                allBullets.Add(new BulletThree(sizes, 15, new Point(DrawCoord.X - sizes.renderingOffsetT.X, DrawCoord.Y - sizes.renderingOffsetT.Y), listDrawns.ListOBJ.Count, nearestMonster));
